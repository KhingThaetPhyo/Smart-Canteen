...
// import 'package:smartcanteen/view/menu_loading_screen.dart';
...
//   future: _futureShops,
//   builder: (context, snapshot) {
//     // 1. LOADING STATE: Return Skeleton Loader
//     if (snapshot.connectionState == ConnectionState.waiting) {
//       return const SliverToBoxAdapter(
//         child: MenuLoadingScreen()
//       );
//     } 
//     // 2. ERROR STATE
//     else if (snapshot.hasError) {
...
//     // 4. DATA LOADED: Return Actual Shop List
//     return SliverList(
//       delegate: SliverChildBuilderDelegate(
//         (context, index) {
//           final shop = shops[index];
...
import 'package:smartcanteen/view/home/shop.dart';
import 'package:smartcanteen/view/menu_loading_screen.dart';
import 'package:smartcanteen/view/network_error_dialog.dart';
import 'package:smartcanteen/view/order_screen.dart';
import 'package:smartcanteen/view/shop_detail_screen.dart';

class Homescreen extends StatefulWidget {
  final VoidCallback? onSeeAllOrdersPressed;

  const Homescreen({
    super.key,
    this.onSeeAllOrdersPressed,
  });
...
class _HomescreenState extends State<Homescreen> {
  ...
  // Keys to invoke refresh on child stateful widgets
  final GlobalKey<HomeHeaderState> _headerKey = GlobalKey<HomeHeaderState>();
  final GlobalKey<MenuSectionState> _menuKey = GlobalKey<MenuSectionState>();
  ...
  // /// Refreshes Header, Menus, Orders, AND Shops simultaneously during pull-down
  // Future<void> _refreshAllData() async {
  //   setState(() {
  //     // 1. Re-fetch the shops list from API
  //     _futureShops = _apiService.getShops(); 
  //   });

  //   // 2. Wait for header user data, popular menus, and orders to reload concurrently
  //   await Future.wait([
  //     _headerKey.currentState?.loadUserData() ?? Future.value(),
  //     _menuKey.currentState?.refreshMenu() ?? Future.value(),
  //     _checkLoginStatusAndFetchOrders(),
  //   ]);
  // }
Future<void> _refreshAllData() async {
    try {
      ...
      if (mounted) {
       // showNetworkErrorDialog(context, () {
          _refreshAllData(); // Retry when button is tapped
        //});
      }
    }
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: RefreshIndicator(
          ...
          onRefresh: _refreshAllData, // <-- Triggers full-page pull-to-refresh
          child: CustomScrollView(
            physics: const AlwaysScrollableScrollPhysics(
              parent: BouncingScrollPhysics(),
            ),
            slivers: [
              ...
              SliverPersistentHeader(
                ...
                delegate: _StickyHeaderDelegate(
                  height: 330,
                  child: HomeHeader(key: _headerKey), // <-- Key attached
                ),
              ),
              ...
              /// POPULAR MENU
              SliverToBoxAdapter(
                child: MenuSection(key: _menuKey), // <-- Key attached
              ),
              ...
              /// 3. CONDITIONAL RECENT ORDERS SECTION
              if (!_isLoadingAuth && _isLoggedIn && _futureTodayOrders != null) ...[
                FutureBuilder<List<OrderModel>>(
                  future: _futureTodayOrders,
                  builder: (context, snapshot) {
