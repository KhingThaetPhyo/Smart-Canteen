...
//   final RecipientModel? initialRecipient;
//   // 1. Make callback optional with '?'
...
//     this.onTransferCompleted, // 2. Remove 'required'
//   });
...
// // class TransferPointScreen extends StatefulWidget {
// //   final int currentBalance;
// //   final RecipientModel? initialRecipient; // Added optional initial recipient
// //   final Function(int amount, String recipient) onTransferCompleted;
...
//     // Automatically pre-populate recipient if scanned from QR
//     if (widget.initialRecipient != null) {
//       _selectedRecipient = widget.initialRecipient;
//       _recipientController.text = widget.initialRecipient!.name;
//     }
...
// //     super.key,
// //     required this.currentBalance,
// //     required this.onTransferCompleted, required RecipientModel initialRecipient,
// //   });
...
//   // Safe call using ?.call()
//   widget.onTransferCompleted?.call(amount, recipient);
...
import 'package:smartcanteen/service/shared_preferences_service.dart'; // Import SharedPreferencesService
...
class _TransferPointScreenState extends State<TransferPointScreen> {
  ...
  // Added state to hold and manage the active current balance dynamically
  late int _activeBalance;

  @override
  void initState() {
    super.initState();
    // Initialize with passed widget parameter first
    _activeBalance = widget.currentBalance;

    // Load active balance dynamically from local storage or API
    _loadUserBalance();
  ...
  /// Fetch user wallet balance from local storage or remote API
  Future<void> _loadUserBalance() async {
    try {
      // 1. Try reading stored wallet balance first
      final wallet = await SharedPreferencesService.getUserWallet();
      if (wallet != null && mounted) {
        setState(() {
          _activeBalance = wallet.balance;
        });
      }

      // 2. Fetch fresh wallet balance from API
      final balanceData = await _apiService.getWalletBalance();
      if (balanceData != null && balanceData['balance'] != null && mounted) {
        setState(() {
          _activeBalance = (balanceData['balance'] as num).toInt();
        });
      }
    } catch (e) {
  ...
  void _handleTransfer() {
    ...
    // Validate using the dynamically loaded balance
    if (amount > _activeBalance) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("လက်ကျန် ပွိုင့် မလုံလောက်ပါ!")),
      );
      return;
  ...
  void _executeFinalTransfer(int amount, String recipient) async {
    String phone = _selectedRecipient != null ? _selectedRecipient!.phone : recipient;
    final pin = _pinController.text.trim();
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: Row(
                          ...
                          textBaseline: TextBaseline.alphabetic,
                          children: [
                            // Displays dynamic state balance
                            Text(
                              NumberFormat('#,###').format(_activeBalance),
                              style: const TextStyle(
                                color: Colors.white,
                                fontSize: 22,
                                fontWeight: FontWeight.bold,
