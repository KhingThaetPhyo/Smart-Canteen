...
//     // 2. Load stored wallet data from SharedPreferences
//     _loadBalanceFromStorage();

//     WidgetsBinding.instance.addPostFrameCallback((_) {
//       final amount = _amount;
//       if (amount != null && _currentBalance < amount) {
//         _showInsufficientBalanceDialog(amount);
//       }
//     });
//   }

//   /// Loads stored wallet from SharedPreferences
//   Future<void> _loadBalanceFromStorage() async {
//     try {
//       final wallet = await SharedPreferencesService.getUserWallet();
//       if (wallet != null && mounted) {
//         setState(() {
//           // Assuming your WalletModel has a balance property (e.g., wallet.balance)
//           _currentBalance = wallet.balance ?? _currentBalance;
//         });
//       }
//     } catch (e) {
//       debugPrint("Error loading wallet from SharedPreferences: $e");
//     }
//   }
...
//     if (amount == null || amount <= 0 || recipient.isEmpty) {
//       _showSnackBar('Please enter valid transfer details.');
//       return;
//     }
...
//     if (confirmed == true && mounted) {
//       await _submitOrderAndTransfer(amount, recipient);
//     }
//   }
...
//     // Format items payload ensuring strict "menu_id" and "quantity" integer keys
//     final List<Map<String, dynamic>> itemsPayload = widget.cartItems.map((item) {
//       final dynamic rawId = item["menu_id"] ?? item["menu_item_id"] ?? item["id"];
//       final dynamic rawQty = item["quantity"] ?? item["cartQuantity"];
...
//       } else {
//         _showApiErrorDialog(
//           response?['message'] ?? 'အော်ဒါ မှာယူမှု မအောင်မြင်ပါ။',
//         );
//       }
//     } catch (e) {
//       setState(() => _isLoading = false);
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                Text(
                  'သင့်လက်ကျန်ပွိုင့် (${NumberFormat('#,###').format(_currentBalance)} ပွိုင့်) သည် ပေးချေလိုသောပွိုင့် (${NumberFormat('#,###').format(requiredAmount)} ပွိုင့်) ထက် နည်းနေပါသည်။',
                  textAlign: TextAlign.center,
                  style: const TextStyle(
                    fontSize: 13,
                    color: Color(0xff64748B),
                ...
                children: [
                  Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        '${NumberFormat('#,###').format(_currentBalance)} ပွိုင့်',
                        style: const TextStyle(
                          color: Colors.white,
                          fontSize: 27,
                          fontWeight: FontWeight.bold,
